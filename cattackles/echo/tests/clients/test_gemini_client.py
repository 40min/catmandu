...
class TestGeminiClient:
    ...
    def test_initialization_success(self, test_gemini_api_key, test_gemini_model):
        ...
        with (
            ...
            client = GeminiClient(api_key=test_gemini_api_key, model_name=test_gemini_model)

            assert client.api_key == test_gemini_api_key
            assert client.model_name == test_gemini_model
            assert client.model == mock_model
            mock_configure.assert_called_once_with(api_key=test_gemini_api_key)
            mock_model_class.assert_called_once_with(test_gemini_model)

    def test_initialization_failure(self, test_gemini_api_key, test_gemini_model):
        ...
        with patch("google.generativeai.configure", side_effect=Exception("API Error")):
            with pytest.raises(RuntimeError, match="Failed to configure Gemini API"):
                GeminiClient(api_key=test_gemini_api_key, model_name=test_gemini_model)

    @pytest.mark.asyncio
    async def test_generate_content_success(self, test_gemini_api_key, test_gemini_model):
    ...
    @pytest.mark.asyncio
    async def test_generate_content_strips_whitespace(self, test_gemini_api_key, test_gemini_model):
    ...
    @pytest.mark.asyncio
    async def test_generate_content_no_model(self, test_gemini_api_key, test_gemini_model):
    ...
    @pytest.mark.asyncio
    async def test_generate_content_api_error(self, test_gemini_api_key, test_gemini_model):
    ...
    def test_default_model_name(self, test_gemini_api_key):
        """Test that default model name is used when not specified."""
        with patch("google.generativeai.configure"), patch("google.generativeai.GenerativeModel") as mock_model_class:

            mock_model_class.return_value = MagicMock()

            client = GeminiClient(api_key=test_gemini_api_key)
